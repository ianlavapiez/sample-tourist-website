...
var displayMap = function displayMap(locations) {
  mapboxgl.accessToken = 'pk.eyJ1IjoibHVmZnlwaXJhdGVzIiwiYSI6ImNqeG56d2QzNDAxcXozZXA0dWpndjFlMXUifQ._9zhfl0f2WVh5_shJbJZ4A';
  ...
  locations.forEach(function (location) {
    ...
    new mapboxgl.Marker({
      element: element,
      anchor: 'bottom'
    }).setLngLat(location.coordinates).addTo(map); // Add pop-up

    new mapboxgl.Popup({
      offset: 30
    }).setLngLat(location.coordinates).setHTML("<p>Day ".concat(location.day, ": ").concat(location.description, "</p>")).addTo(map); // Extends map bounds to include current location
